...
#CODE FOR TRAINING THE RESNET18 MODEL FROM PYTORCH WITH pretrained=False
...
learning_rate = 0.0001 #first try was 0.001 with terrible results
batch_size = 100
epochs = 50
...
dataset = ZTF_lightkurve_img(annotations_file = annotations_dir, img_dir = img_dir) #in the classification paper we have ~ 47 labels)
...
ZTF_model.fc = torch.nn.Linear(ZTF_model.fc.in_features, 31) #change the last layer to correspond to our label number
loss_fn = nn.BCEWithLogitsLoss() #this normalises logits so we don't need to use softmax
optimizer = torch.optim.SGD(ZTF_model.parameters(), lr = learning_rate)

#data parallelisation implementation
...
for t in range(epochs):
    print("Epoch {}\n----------------------------------------------".format(t + 1))
    epoch_list.append(t)
    model_evaluation_dict_train = train_loop(train_loader, ZTF_model, loss_fn, optimizer, device) #remember to create file to write data into
...
#save our model parameters to some file
torch.save(ZTF_model, 'models/ZTF_model_res.pth')
...
for i in range(len(model_evaluation_keys)): #length of 5
    if i<2:
        ...
        if row == 10:
            break
#plt.tight_layout()
plt.savefig('model_evaluation_plots/all_in_one_res.jpg', dpi=300)
plt.close()
...
for i in range(len(training_df_cols)):
    ...
    if row == 31:
        break
plt.savefig('model_evaluation_plots/class_confusion_matrix_res.jpg', dpi=300)
plt.close()
