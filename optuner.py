...
#the code skeleton is from the optuna tutorial modified to be applied for this work (or claude.ai)
#hyperparameters to be optimized: optimizer, learning rate, batch size, momentum
...
def load_best_metric(filepath='optuna_models/best_model.txt'):
...
def save_best_metric(filepath='optuna_models/best_model.txt', trial_number=None, metric=None,
                     loss=None, prec_recall=None):
    with open(filepath, 'w') as file:
        ...
        file.write(f"Best prec_recall: {prec_recall}\n")
...
def objective(single_trial, device_id):
    ...
    ZTF_model = ZTF_model.to(device_id)
    ...
    #parallelisation code (help from claude.ai)
    ...
    for t in range(epochs):
        ...
        #this is the sum of the individual precisions
        evaluation_score_precision = model_evaluation_dict["precision"].sum().item()
        evaluation_score_recall = model_evaluation_dict["recall"].sum().item()
        prec_recall_score = evaluation_score_precision + evaluation_score_recall


        #combine the metrics for easier finding of the best model
        combined_metric = loss_score - prec_recall_score


        if dist.get_rank() == 0:
            best_val_loss = load_best_metric()
            if combined_metric < best_val_loss:
                torch.save(ZTF_model.state_dict(), 'optuna_models/ZTF_model_optuna.pth')
                save_best_metric('optuna_models/best_model.txt', trial_number=trial.number,
                                 metric=combined_metric, loss = loss_score, prec_recall= prec_recall_score)

        #for pruning we only use loss_score for now (or we could use the combined metric but loss it is)
...
if __name__ == "__main__":
    ...
    with open("optuna_models/study_stats.txt", 'w') as file:
    ...
    df.to_csv('optuna_models/optuna_results.csv', index=False)  # Save to csv file
    ...
    try:
        ...
        fig_optimization_history.savefig("optuna_plots/optimization_history.png", dpi=300)
        plt.close()
    ...
    try:
        ...
        fig_intermediate_values.savefig("optuna_plots/intermediate_values.png", dpi=300)
        plt.close()
    except Exception as e:
    ...
    try:
        ...
        fig_parallel_coordinate.savefig("optuna_plots/parallel_coordinate.png", dpi=300)
        plt.close()
    except Exception as e:
    ...
    try:
        ...
        fig_contour.savefig("optuna_plots/contour.png", dpi=300, bbox_inches='tight')
        plt.close()
    except Exception as e:
    ...
    try:
        ...
        fig_slice.savefig("optuna_plots/slice.png", dpi=300, bbox_inches='tight')
        plt.close()
    except Exception as e:
    ...
    try:
        ...
        fig_param_importances.savefig("optuna_plots/param_importances.png", dpi=300)
        plt.close()
    except Exception as e:
    ...
    try:
        ...
        fig_timeline.savefig("optuna_plots/timeline.png", dpi=300)
        plt.close()
    except Exception as e:
    ...
    state_dict = torch.load("optuna_models/ZTF_model_optuna.pth")
    ...
    xx, yyy = test_loop(test_loader, ZTF_model_best, trial.params["batch_size"], loss_fn,
                        training_csv_path, 'optuna_models/best_model_score', trial.params["threshold"], devicers)

    with open('optuna_models/ZTF_model_test.txt', 'w') as file:
